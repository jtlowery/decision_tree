...
class DtreeSuite extends FunSuite {
  ...
  test("dataprep - simple cases") {
    assert(
      prepareData(rawData = Vector(Vector(1)), labels = Vector(1)) ===
        Vector(TreeData(features = Vector(1), label = 1, rowIndex = 0))
    )
    assert(
      prepareData(rawData = Vector(Vector(1, 1), Vector(2, 1)), labels = Vector(1, 0)) ===
  ...
  test("find best split on a col -- splitVariable") {
    val d1 = Vector(1.0, 1.0, 1.0)
    val d2 = Vector(1.0, 1.0, 0.0)
    val d3 = Vector(0.0, 0.0, 1.0)
    val d4 = Vector(1.0, 0.0, 0.0)
    val d = Vector(d1, d2, d3, d4)
    val labels = Vector(1, 1, 2, 2)
    val dt = prepareData(rawData = d, labels = labels)
  ...
  test("splitContinuous - basic case") {
    val fd1 = FeatureDataPoint(feature = 1.0, label = 1, rowIndex = 0, colIndex = 0)
    val fd2 = FeatureDataPoint(feature = 1.1, label = 1, rowIndex = 1, colIndex = 0)
    val fd3 = FeatureDataPoint(feature = 1.7, label = 2, rowIndex = 2, colIndex = 0)
    val fd4 = FeatureDataPoint(feature = 2.2, label = 2, rowIndex = 3, colIndex = 0)
    val split = splitContinuous(Vector(fd1, fd2, fd3, fd4), minSamplesSplit = 1)
    assert(split.get.leftDataRowIndexes === Vector(0, 1))
    assert(split.get.rightDataRowIndexes === Vector(2, 3))
    assert(split.get.iGain === infoGain(Vector(1, 1, 2, 2), Vector(1, 1), Vector(2, 2)))
    assert(split.get.colIndex === 0)
    assert(split.get.predicate(1.1) === true)
    assert(split.get.predicate(1.7) === false)
  }
  ...
  test("test terminate splitting on entropy") {
    ...
    val d3 = Vector(0.0, 0.0, 1.0)
    val d = Vector(d1, d2)
    val labels = Vector(1, 1)
    val zeroEntropyData = prepareData(rawData = d, labels = labels)
    assert(terminateSplitting(zeroEntropyData, depth = 1, maxDepth = 10) === true)

    val dd = Vector(d1, d2, d3)
    val labels2 = Vector(1, 1, 2)
    val nonZeroEntropyData = prepareData(rawData = dd, labels = labels2)
  ...
  test("test terminate splitting on depth") {
    ...
    val d3 = Vector(0.0, 0.0, 1.0)
    val d = Vector(d2, d3)
    val labels = Vector(1, 2)
    val nonZeroEntropyData = prepareData(rawData = d, labels = labels)
  ...
  test("predict with a tree of two leaves and a single branch") {
    ...
    val testTree = Branch(left = Leaf(1), right = Leaf(2), depth = 0, informationGain = 1.0, splitCol = 1,
      splitPredicate = (x: Double) => x == 1, dataRowIndexes = Vector(0,1,2,3))
    assert(predict(testTree, d1) === 1)
    assert(predict(testTree, d2) === 1)
    assert(predict(testTree, d3) === 2)
    assert(predict(testTree, d4) === 2)

    val testTree2 = Branch(Leaf(2), Leaf(1), 0, 1.0, 1, (x: Double) => x == 0, Vector(0,1,2,3))
